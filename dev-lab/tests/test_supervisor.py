...
def test_serve_processes_jobs(tmp_path):
    ...
    n = asyncio.run(serve(config=Config(), queue=q, max_jobs=2, run=fake))
...
def test_serve_failing_job_moves_to_failed(tmp_path):
    ...
    n = asyncio.run(serve(config=Config(), queue=q, max_jobs=1, run=boom))
...
def test_serve_job_without_repo_fails(tmp_path):
    ...
    n = asyncio.run(
        serve(config=Config(), queue=q, default_repo=None, max_jobs=1, run=fake)
    )
...
def test_serve_records_runs_in_db(tmp_path):
    ...
    asyncio.run(serve(config=Config(), queue=q, max_jobs=2, db=conn, run=run))
    ...
    async def scenario():
        async with bus.subscribe() as sub:
            task = asyncio.create_task(
                serve(config=Config(), queue=q, max_jobs=1, bus=bus, run=fake)
            )
            while True:
...
def test_serve_recovers_inflight_job(tmp_path):
    ...
    n = asyncio.run(serve(config=Config(), queue=q, max_jobs=1, run=fake))
